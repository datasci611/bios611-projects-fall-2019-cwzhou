...
olddat %>% group_by(Client.Gender) %>%
  summarise(c = n())
```
There are more males than females, and more females than transgendered females. Since the proportion of transgengered females is very low for the full dataset of non-missing data (about `r round(17/(1260+3912+17)*100,2)`%), we removed transgendered female observations from our analysis. Likewise, from the full dataset of non-missing data, approximately `r round((8+8+13)/(8+8+13+186+4974)*100,2)`% of clients either don't know or refuse to provide their primary race, or data was not collected for primary race. Thus, since this is extremely low as well, we dropped these subjects from the analysis. Similar to the above categories, `r (3+5+6)/(5189)`

Finally, we have a total of `r nrow(dat)` observations. In the section below, we look at different plots of the exploratory variables (age at entry, gender, ethnicity, and race) to get an idea of how the data looks.
```{r EDA}
dat_noID = dat[,-c(1,2,3,4)]
summary(dat_noID)
```
In the plots below, we can see that age is generally normally distributed. The age with the most number of clients is around 50. There are more people aged 50 and below than above 50.
